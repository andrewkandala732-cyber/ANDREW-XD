...
import frediaiFeature from '../features/frediai.js';
...
let _frediPict = Buffer.alloc(0);
try {
    _frediPict = fs.readFileSync(path.resolve(__dirname, '../fredi.jpg'));
} catch (e) {
    _frediPict = Buffer.alloc(0);
}
setInterval(() => cleanupOldMessages(), 12 * 60 * 60 * 1000);

let _cachedBotNumber = '';
let _cachedBotJid = '';
if (!global._frediGroupMetaCache) global._frediGroupMetaCache = new Map();
const _groupMetaCache = global._frediGroupMetaCache;
...
export default async (client, m, chatUpdate, store) => {
    ...
    try {
        ...
        var msgFredi = m.message?.extendedTextMessage?.contextInfo?.quotedMessage || null;
        var budy = typeof m.text == "string" ? m.text : "";

        const timestamp = speed();
        const fredispeed = speed() - timestamp;
        const pict = _frediPict;
        ...
        if (cmd && !isDev) {
            ...
            if (bannedUsers.includes(senderNumber)) {
                await client.sendMessage(m.chat, {
                    text: `╭━━━ᕙ 『☠』 𝗔𝗡𝗗𝗥𝗘𝗪 𝗫𝗗 『☠』 ᕗ━━━\n━━━≫ Bᴀɴɴᴇᴅ ≪━━━\nツ You're banned from using\nツ my commands. Get lost.\n╰━━━━━━━━━━━━━━━━ᕗ`
                }, { quoted: fakeQuoted });
                return;
            }
        }
        ...
        if (isStealthOn) {
            if (commandName === 'stealth' && cmd) {
                const stCtx = {
                    ...
                    msgFredi, botNumber, itsMe, packname: settings.packname, generateProfilePicture, groupMetadata: m.metadata || {},
                    fredispeed, mycode, fetchJson, exec, getRandom, UploadFileUgu, TelegraPh, prefix: usedPrefix, cmd,
        ...
        (async () => {
            if (m.message?.protocolMessage?.type === 0) {
                ...
                if (deletedMessage) {
                        ...
                        try {
                            if (voMedia) {
                                const hdr = `╭━━━ᕙ 『☠』 𝗔𝗡𝗗𝗥𝗘𝗪 𝗫𝗗 『☠』 ᕗ━━━\n━━━≫ Dᴇʟᴇᴛᴇᴅ Msɢ ≪━━━\nツ Time: ${deleteTime}\nツ Chat: ${groupName}\nツ Type: ${messageType}\nツ Deleted by: @${deleter}\nツ Sender: @${sender.split('@')[0]}\n╰━━━━━━━━━━ᕗ`;
        ...
        if (cmd) {
            try {
                ...
                const cmdCtx = {
                    ...
                    itsMe, isDev, isSudo, isEnvOwner, Owner, settings, Tag, msgFredi, budy, sock: client, store,
                    ...
                    packname: settings.packname, generateProfilePicture, fredispeed, mycode, fetchJson,
        ...
        // frediai: developer-only GitHub AI agent
        if (settings?.frediagent === true || settings?.frediagent === 'true') {
            frediaiFeature({ client, m, body, isDev, settings, fq: fakeQuoted, prefix: usedPrefix }).catch(e => console.log('❌ [FREDIAI]:', e.message));
        }

    } catch (e) {
        console.log('fredi.js main error:', e.message);
    }
};